                ...
                if transactions.isEmpty {
                    ...
                    ForEach(transactions.prefix(5)) { transaction in
                        ...
                        DashboardTransactionItem(
                            title: transaction.storeName ?? cat?.name ?? "Unknown",
            ...
            VStack(spacing: 16) {
                if budgets.isEmpty {
                    ...
                    ForEach(budgets.prefix(3)) { budget in
                        let cat = categories.first(where: { $0.id == budget.categoryId })
                        budgetProgressItem(title: cat?.name ?? "Budget", spent: budget.currentSpent, total: budget.limitAmount, color: cat?.color ?? .blue)
                    }
                }
            }
            .padding()
